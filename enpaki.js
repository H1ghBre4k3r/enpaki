...
const { BUNDLE_HEADER, BUNDLE_FOOTER, FILE_HEADER, FILE_FOOTER } = require("./bundle-template.js");
...
module.exports = class Enpaki extends Readable {
    ...
    getCompiler(extension) {
        const compiler = this.compilers[extension] || {
            compile: filename => {
        ...
        if (!this.parsedFiles.includes(filename)) {
            ...
            if (moduleIdentity.includes("node_modules")) {
                ...
                while (!locate.isFile(pkgFile) && pkgFile !== "/package.json") {
                    pkgFile = path.join(path.dirname(pkgFile), `..${path.sep}`, "package.json");
                }
